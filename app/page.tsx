...
const ProductCard = ({
  ...
  >
    ...
    <div className="h-[40%] p-6 md:p-8 flex flex-col justify-between relative bg-white group-hover:bg-slate-50/50 transition-colors">
      {/* Floating Icon */}
      <div className="absolute -top-6 md:-top-8 right-6 md:right-8 h-12 w-12 md:h-16 md:w-16 bg-gradient-to-br from-cobalt to-indigo-600 text-white rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg shadow-cobalt/30 group-hover:-translate-y-2 transition-transform duration-300">
        <ArrowUpRight size={24} className="md:w-7 md:h-7" />
      </div>

      <div>
        <div className="flex items-center gap-3 mb-2 md:mb-3">
          <div className="h-[2px] w-6 bg-gradient-to-r from-cobalt to-cyan-400" />
          <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-400">
            Product Line
          </span>
        </div>
        <h3 className="text-xl md:text-2xl font-display font-bold text-slate-900 leading-none mb-2 md:mb-3 group-hover:text-cobalt transition-colors">
          {title}
        </h3>
      ...
      <div className="flex items-center justify-between border-t border-slate-100 pt-4 md:pt-6 mt-2">
        <span className="text-[10px] md:text-xs font-bold uppercase text-slate-900 group-hover:text-cobalt transition-colors">
          View Specifications
        </span>
        <ArrowRight
          size={14}
          className="text-slate-300 group-hover:text-cobalt transform group-hover:translate-x-2 transition-all"
        />
      </div>
    </div>
  </motion.div>
...
const ServiceCard = ({
  ...
  >
    ...
    <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end z-10">
      {/* Top Border Accent */}
      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-cobalt to-cyan-400 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-700" />
      ...
      <div className="transform translate-y-4 md:translate-y-8 group-hover:translate-y-0 transition-transform duration-500">
        ...
        <div className="mb-4 md:mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100 flex flex-wrap gap-2">
          {specs.map((s, i) => (
            <span
              key={i}
              className="px-2 py-1 border border-cobalt/50 bg-cobalt/10 text-cyan-300 text-[9px] md:text-[10px] font-mono font-bold uppercase tracking-wider rounded backdrop-blur-md"
            >
              {s}
            </span>
          ))}
        ...
        <h3 className="text-2xl md:text-4xl font-display font-bold text-white mb-2 md:mb-3 leading-none group-hover:text-cyan-400 transition-colors">
          {title}
        </h3>
        <div className="h-0.5 w-12 bg-gradient-to-r from-cobalt to-cyan-400 mb-3 md:mb-4 group-hover:w-full transition-all duration-500" />
        <p className="text-slate-300 text-xs md:text-base mb-6 md:mb-8 line-clamp-2">{subtitle}</p>

        <Link
          href={link}
          className="inline-flex items-center justify-between w-full p-3 md:p-4 rounded-xl bg-white/5 border border-white/10 group-hover:bg-cobalt group-hover:border-cobalt transition-all group-hover:shadow-[0_0_20px_rgba(37,99,235,0.4)] backdrop-blur-sm"
        >
...
const ProjectThumb = ({ title, location, image }: { title: string; location: string; image: string }) => (
  <div className="group relative aspect-[4/5] rounded-[1.5rem] md:rounded-3xl overflow-hidden cursor-pointer shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
    ...
    <div className="absolute bottom-0 left-0 p-6 md:p-8 w-full">
      <div className="h-1 w-8 bg-gradient-to-r from-cobalt to-cyan-400 mb-3 md:mb-4 group-hover:w-16 transition-all duration-300" />
          ...
          <div className="container mx-auto max-w-7xl relative z-10">
            <SectionReveal className="flex flex-col lg:flex-row items-end gap-10 lg:gap-16 mb-16 md:mb-24">
              <div className="lg:w-3/5">
                <div className="flex items-center gap-4 mb-4 md:mb-6">
                  <div className="h-[2px] w-8 md:w-12 bg-gradient-to-r from-cobalt to-cyan-400"></div>
                ...
                <h2 className="font-display text-4xl sm:text-5xl md:text-8xl font-bold text-ink leading-[0.9] tracking-tight">
                  The Invisible <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-cobalt via-indigo-600 to-cyan-500">
                    Framework.
                  </span>
                </h2>
              </div>
              <div className="lg:w-2/5 pb-0 md:pb-4">
                <p className="text-base md:text-xl text-slate-600 leading-relaxed font-medium mb-8 md:mb-10 border-l-2 border-slate-200 pl-4 md:pl-6">
                  MAG Group engineers the essential skeletal and circulatory systems of the modern city. From unseen
                  drainage networks to the steel bones of skyscrapers.
                </p>
                <div className="flex gap-4 pl-4 md:pl-6">
                  <div className="flex -space-x-3 md:-space-x-4">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-10 h-10 md:w-12 md:h-12 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center overflow-hidden shadow-sm"
                      >
            ...
            <SectionReveal className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {[
                ...
                <div
                  key={i}
                  className="group bg-white p-5 md:p-8 rounded-[1.5rem] md:rounded-[2rem] border border-slate-100 hover:border-cobalt/30 transition-all hover:-translate-y-2 hover:shadow-xl hover:shadow-cobalt/5 relative overflow-hidden"
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-3xl md:text-5xl font-display font-bold text-ink mb-1 md:mb-2">
                      {stat.val}
                      <span className="text-sm md:text-lg text-cobalt ml-1">{stat.unit}</span>
                    </p>
          ...
          <div className="container mx-auto px-4 md:px-6 relative z-10">
            <SectionReveal className="mb-12 md:mb-16 text-center max-w-3xl mx-auto">
              <div className="inline-flex items-center gap-2 bg-white px-3 py-1.5 md:px-4 md:py-2 rounded-full shadow-md shadow-slate-200/50 mb-6 md:mb-8 border border-slate-100">
                ...
                <span className="text-slate-500 font-mono text-[10px] md:text-xs font-bold uppercase tracking-widest">
                  Division 01
                </span>
              </div>
              <h2 className="font-display text-4xl md:text-6xl font-bold text-slate-900 mb-4 md:mb-6">
                Drainage Products
              </h2>
            ...
            <div className="mt-12 md:mt-16 text-center">
              <Link href="/products">
                <Button
                  variant="outline"
                  className="rounded-full bg-white border-slate-200 text-slate-900 hover:border-cobalt hover:bg-cobalt hover:text-white px-8 md:px-10 h-12 md:h-14 text-xs md:text-sm shadow-xl shadow-slate-200 transition-all hover:scale-105"
                >
                  View Full Catalog
                </Button>
              </Link>
            ...
            <SectionReveal className="flex flex-col md:flex-row justify-between items-end gap-6 md:gap-8 mb-12 md:mb-20 pb-8 border-b border-white/10">
              <div className="max-w-2xl">
                <div className="flex items-center gap-3 mb-4 md:mb-6">
                  <div className="px-3 py-1 bg-gradient-to-r from-cobalt to-indigo-600 text-white rounded-full text-[9px] md:text-[10px] font-bold uppercase tracking-widest shadow-[0_0_15px_rgba(37,99,235,0.5)]">
                    Division 02
                  </div>
                  <span className="text-slate-400 font-mono text-[10px] md:text-xs uppercase tracking-widest border border-slate-700 px-3 py-1 rounded-full bg-slate-800/50">
              ...
              <div className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] backdrop-blur-md hover:bg-white/10 transition-colors w-full md:w-auto">
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl md:text-5xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-r from-cobalt to-cyan-400">
                    35k
                  </span>
                  <span className="text-xs md:text-sm font-bold text-white uppercase">Tons / Year</span>
                </div>
                <p className="text-[10px] md:text-xs text-slate-400 mt-2 uppercase tracking-wider">
    ...
    />
            ...
            <div className="mt-12 md:mt-16 text-center">
              <Link href="/services">
                <Button
                  variant="outline"
                  className="border-slate-600 text-white hover:bg-white hover:text-black hover:border-white h-12 md:h-14 px-10 md:px-12 rounded-full backdrop-blur-sm bg-white/5 text-xs md:text-sm"
                >
          ...
          <div className="container mx-auto px-4 md:px-6 relative z-10">
            ...
            <SectionReveal className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-20 items-center mb-16 md:mb-24">
              <div className="order-2 lg:order-1 relative">
                <div className="relative rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-2xl shadow-indigo-200 border-4 border-white h-[300px] md:h-auto">
                ...
                <div className="absolute -bottom-6 -right-4 md:-bottom-8 md:-right-8 bg-white p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] shadow-xl border border-slate-50 max-w-[200px] md:max-w-xs hidden sm:block">
                  ...
                  <p className="text-[10px] md:text-xs text-slate-500 leading-relaxed">
                    We control the entire value chain from design to fabrication, ensuring zero compromise on quality.
                  </p>
                </div>
              </div>

              <div className="order-1 lg:order-2">
                <div className="inline-flex items-center gap-2 mb-4 md:mb-6 px-3 py-1.5 md:px-4 md:py-2 bg-indigo-50 rounded-full border border-indigo-100">
                  <Factory className="text-indigo-600 w-3 h-3 md:w-4 md:h-4" />
                  <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-indigo-700">
                    Manufacturer, Not Just Supplier
                  </span>
                </div>
                <h2 className="font-display text-4xl md:text-6xl font-bold text-ink mb-4 md:mb-6 leading-tight">
                  Total Control Over <br />
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-cobalt to-indigo-600">
                    Every Detail.
                  </span>
                </h2>
                <p className="text-base md:text-lg text-slate-600 leading-relaxed mb-6 md:mb-8">
                  Unlike trading companies that rely on third parties, MAG Group is a true manufacturer. We possess the
                  engineering depth to customize products to your exact project requirements, whether it's a bespoke
                  drain for a luxury villa or a 50m clear-span truss for an aircraft hangar.
                </p>
            ...
            <SectionReveal>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                <ProjectThumb title="Lusail Stadium" location="Doha, Qatar" image={IMAGES.projects.lusail} />
                <ProjectThumb title="Museum of Future" location="Dubai, UAE" image={IMAGES.projects.museum} />
                <ProjectThumb title="Hamad Int. Airport" location="Doha, Qatar" image={IMAGES.projects.airport} />
                <ProjectThumb title="Yas Marina Circuit" location="Abu Dhabi, UAE" image={IMAGES.misc.qatar_office} />
              </div>
          ...
          <div className="container mx-auto px-4 md:px-6 text-center relative z-10">
            <SectionReveal>
              ...
              <p className="text-base md:text-xl text-slate-500 mb-10 md:mb-12 max-w-2xl mx-auto">
                Partner with the region's leading manufacturer for your next project. Get expert consultation for custom
                fabrication and drainage systems.
              </p>
              <div className="flex flex-col sm:flex-row justify-center gap-4 md:gap-6">
                <Link href="/contact">
                  <Button
                    size="lg"
                    className="bg-cobalt text-white hover:bg-blue-700 h-14 md:h-16 px-10 md:px-12 text-sm md:text-base rounded-full shadow-2xl shadow-blue-500/30 hover:-translate-y-1 transition-all w-full sm:w-auto"
                  >
                    Contact Us
                  </Button>
                </Link>
                <Link href="/about">
                  <Button
                    variant="outline"
                    size="lg"
                    className="h-14 md:h-16 px-10 md:px-12 text-sm md:text-base rounded-full border-slate-200 bg-white hover:bg-slate-50 text-slate-700 w-full sm:w-auto"
                  >
                    Our Locations
                  </Button>
                </Link>
